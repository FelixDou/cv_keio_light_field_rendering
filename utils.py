...
def bulldozer_light_field_4d(s: float, t: float, u: float, v: float, data) -> set:
    ...
    # Plotting the light field for each color
    plt.figure()
    plt.plot(coords, blue, "o", label="blue")
    plt.plot(coords, green, "o", label="green")
    plt.plot(coords, red, "o", label="red")
    plt.legend()
    plt.show()
...
def bulldozer_light_field_renderer_movable(
    x: float,
    y: float,
    z: float,
    theta: float,
